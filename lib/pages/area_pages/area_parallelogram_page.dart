...
class AreaParallelogramPage extends StatefulWidget {
  const AreaParallelogramPage({super.key});

  @override
  State<AreaParallelogramPage> createState() => _AreaParallelogramPageState();
}

class _AreaParallelogramPageState extends State<AreaParallelogramPage> {
  ...
  void _showResult(double result) {
    showDialog(
      ...
      builder: (ctx) => AreaResultDialog(
        customIcon: ParallelogramIcon(color: Colors.white, size: 40),
  ...
  void _calculate() {
    if (_formKey.currentState!.validate()) {
      ...
      Future.delayed(const Duration(milliseconds: 200), () {
        ...
        final result = AreaCalculationService.calculateParallelogram(base, height);
        _showResult(result);
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Luas Jajar Genjang'),
        actions: [
          IconButton(
                ...
                child: Row(
                  children: [
                    Container(
                      ...
                      child: ParallelogramIcon(color: Colors.white, size: 28),
                    ),
